...
public interface OrderRepository extends JpaRepository<Order, Long> {
    ...
    /** Sum grand totals of all non-deleted orders. */
    @Query("SELECT COALESCE(SUM(o.grandTotal), 0) FROM Order o WHERE o.deletedAt IS NULL")
    BigDecimal sumGrandTotal();

    /** Sum received amounts of all non-deleted orders. */
    @Query("SELECT COALESCE(SUM(o.receivedAmount), 0) FROM Order o WHERE o.deletedAt IS NULL")
    BigDecimal sumReceivedAmount();

    /** Count non-deleted orders by status. */
    @Query("SELECT COUNT(o) FROM Order o WHERE o.deletedAt IS NULL AND o.status = :status")
    long countByStatus(@Param("status") OrderStatus status);

    /** Count non-deleted orders created after the given timestamp. */
    @Query("SELECT COUNT(o) FROM Order o WHERE o.deletedAt IS NULL AND o.createdAt >= :from")
    long countActiveCreatedAfter(@Param("from") LocalDateTime from);

    /** Sum grand totals for orders created after the given timestamp. */
    @Query("""
            SELECT COALESCE(SUM(o.grandTotal), 0) FROM Order o
            WHERE o.deletedAt IS NULL AND o.createdAt >= :from
            """)
